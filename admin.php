...
$sql2="select * from sample3";
$result2=mysqli_query($con,$sql2);
$sql4="select * from sample5";
...
{
   $info=getData();
   $search_query="SELECT * FROM sample1 WHERE Fname='$info[2]'";
...
{
   $info=getData1();
   $search_query="SELECT * FROM sample1 WHERE SSN='$info[1]'";
   $search_result=mysqli_query($con,$search_query);
   if($search_result)
   {
        if(mysqli_num_rows($search_result))
...
{
     $info=getData1();
     $insert_query="INSERT INTO sample2( SSN, Worked_Days,Off_Days,Extra_Days) VALUES ('$info[1]','$info[2]','$info[3]','$info[4]')";
     try{
          $insert_result=mysqli_query($con,$insert_query);
          if($insert_result){
               if(mysqli_affected_rows($con)>0){
...
{
     $info = getData1();
     $delete_query="DELETE FROM sample2 WHERE SSN='$info[1]'";
     try{
          $delete_result=mysqli_query($con,$delete_query);
          if($delete_result){
               if(mysqli_affected_rows($con)>0){
...
if(isset($_POST['update1'])){
     $info=getData1();
     $update_query="UPDATE sample2 SET SSN='$info[1]',Worked_Days='$info[2]',Off_Days='$info[3]',Extra_Days='$info[4]' WHERE SSN='$info[1]'";
     try{
          $update_result=mysqli_query($con,$update_query);
          if($update_result){
               if(mysqli_affected_rows($con)>0){
...
{
     $info=getData2();
     $insert_query="INSERT INTO sample3( Dno,SSN,Dname,Dlocation) VALUES ('$info[1]','$info[2]','$info[3]','$info[4]')";
     try{
          $insert_result=mysqli_query($con,$insert_query);
          if($insert_result){
               if(mysqli_affected_rows($con)>0){
...
{
     $info = getData2();
     $delete_query="DELETE FROM sample3 WHERE SSN='$info[2]'";
     try{
          $delete_result=mysqli_query($con,$delete_query);
          if($delete_result){
               if(mysqli_affected_rows($con)>0){
...
if(isset($_POST['update2'])){
  $info=getData2();
  $update_query="UPDATE sample3 SET Dno='$info[1]',SSN='$info[2]',Dname='$info[3]',Dlocation='$info[4]' WHERE SSN='$info[2]'";
  try{
       $update_result=mysqli_query($con,$update_query);
       if($update_result){
            if(mysqli_affected_rows($con)>0){
...
{
     $info=getData3();
     $insert_query="INSERT INTO sample4(SSN,Monthly_Salary,Monthly_Deductions,Monthly_Insurances) VALUES ('$info[1]','$info[2]','$info[3]','$info[4]')";
     try{
          $insert_result=mysqli_query($con,$insert_query);
          if($insert_result){
               if(mysqli_affected_rows($con)>0){
...
{
     $info = getData3();
     $delete_query="DELETE FROM sample4 WHERE SSN='$info[1]'";
     try{
          $delete_result=mysqli_query($con,$delete_query);
          if($delete_result){
               if(mysqli_affected_rows($con)>0){
...
if(isset($_POST['update3'])){
  $info=getData3();
  $update_query="UPDATE sample4 SET SSN='$info[1]',Monthly_Salary='$info[2]',Monthly_Deductions='$info[3]',Monthly_Insurances='$info[4]' WHERE SSN='$info[1]'";
  try{
       $update_result=mysqli_query($con,$update_query);
       if($update_result){
            if(mysqli_affected_rows($con)>0){
...
{
     $info=getData4();
     $insert_query="INSERT INTO sample5(Pno,Pname,Dno,SSN) VALUES ('$info[1]','$info[2]','$info[3]','$info[4]')";
     try{
          $insert_result=mysqli_query($con,$insert_query);
          if($insert_result){
               if(mysqli_affected_rows($con)>0){
...
{
     $info = getData4();
     $delete_query="DELETE FROM sample5 WHERE Pno='$info[1]'";
     try{
          $delete_result=mysqli_query($con,$delete_query);
          if($delete_result){
               if(mysqli_affected_rows($con)>0){
...
if(isset($_POST['update4'])){
  $info=getData4();
  $update_query="UPDATE sample5 SET Pno='$info[1]',Pname='$info[2]',Dno='$info[3]',SSN='$info[4]' WHERE Pno='$info[1]'";
  try{
       $update_result=mysqli_query($con,$update_query);
       if($update_result){
            if(mysqli_affected_rows($con)>0){
...
<h1>Employee:</h1>
<form method="post" action="<?php htmlspecialchars($_SERVER["PHP_SELF"]);?>"><br><br>
...
<input type="submit" name="find4" value="Find">
</div>
</form>
...
<input type="submit" name="find3" value="Find">
</div>
</form>
